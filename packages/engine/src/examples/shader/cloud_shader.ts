...
const SkyShader: Shader = {
  ...
  fragmentShader: /* glsl */ `
    ...
    void main() {
      ...
      vec4 tNoise1 = texture2D(tCloudNoise, vec2(vUv.x + uTime * 0.0001, vUv.y - uTime * 0.00014)); // noise txt
      vec4 tNoise2 = texture2D(tCloudNoise, vec2(vUv.x - uTime * 0.00002, vUv.y + uTime * 0.000017 + 0.2)); // noise txt
      float noiseBig = fbm3d(vec3(vUv * uFac1, uTime * uTimeFactor1), 4)+ 1.0 * 0.5;
      newUv += noiseBig * uDisplStrenght1;
      float noiseSmall = noise3d(vec3(newUv * uFac2, uTime * uTimeFactor2));
      newUv += noiseSmall * uDisplStrenght2;
      newUv += vec2(0.0, 0.02);
      vec4 tShape = texture2D(tShape, newUv);
      float alpha = levels((tNoise1 + tNoise2) * 0.6, 0.2, 0.4, 0.7).r;
      alpha *= tShape.r;
