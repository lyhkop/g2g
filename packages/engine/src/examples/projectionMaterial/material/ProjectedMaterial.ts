...
import { createDataTextureFromVec3Array } from "../createDataTexture.js";
...
export class ProjectedMaterial extends MeshPhysicalMaterial {
  ...
  constructor({
    ...
    this.uniforms = {
      ...
      clipPolygonTexture: { value: data },
      clipPolygonPointCount: { value: 4 },
    };
  ...
  #onBeforeCompileCommon: this["onBeforeCompile"] = (shader) => {
    ...
    shader.fragmentShader = monkeyPatch(shader.fragmentShader, {
      header: /* glsl */ `
					...
					uniform sampler2D clipPolygonTexture;
					uniform int clipPolygonPointCount;
      ...
      "vec4 diffuseColor = vec4( diffuse, opacity );": /* glsl */ `
					...
					if (isFacingProjector && isInTexture && isTextureLoaded && isTextureProjected) {
						...
						// add clip polygon
						textureColor.a = isInPolygon(uv, clipPolygonTexture, clipPolygonPointCount) ? textureColor.a : 0.0;
  ...
  #saveCameraMatrices(updateWorldMatrices = true) {
    ...
    this.uniforms.projDirection.value
      .set(0, 0, 1)
      .applyMatrix4(modelMatrixCamera);
...
function getCameraRatio(camera: PerspectiveCamera | OrthographicCamera) {
  switch (camera.type) {
    ...
    default: {
      throw new Error(
        `The given type of camera is currently not supported in ProjectedMaterial`
      );
    }
  }
}
...
export function allocateProjectionData(
...
export function isProjectedMaterial(
  mat: Material | Material[]
): mat is ProjectedMaterial {
  const _mat = mat as ProjectedMaterial | ProjectedMaterial[];
  return Array.isArray(_mat)
    ? _mat.every((m) => m.isProjectedMaterial)
    : _mat.isProjectedMaterial;
}
