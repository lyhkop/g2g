...
uniform vec3 uWIND;
uniform float uFBM_FREQ;

uniform float uCLOUD_OPACITY;


in vec3 vOrigin;
in vec3 vDirection;

#define NO_NOISE_TEXTURE 1
...
float light(
	...
	for (int i = 0; i < steps; i++) {
		float dens = density(pos, uWIND * u_time, 0.);

		float T_i = exp(-uABSORPTION * dens * march_step);
		T *= T_i;
		//if (T < .01) break;
...
vec4 render_clouds(
	...
	sphere_t atmosphere2 = _begin(sphere_t)
	vec3(vOrigin), 100., 0
	_end;

	hit_t hit = no_hit;
	intersect_sphere(eye, atmosphere2, hit);
	//hit_t hit_2 = no_hit;
	//intersect_sphere(eye, atmosphere_2, hit_2);

	float thickness = uTHICKNESS; // length(hit_2.origin - hit.origin);
  ...
  vec3 offset = uWIND * u_time;
	for (int i = 0; i < steps; i++) {
		float h = float(i) / float(steps);
		float dens = density (pos, offset, h);
...
void mainImage2(
  ...
  vec3 sky = render_sky_color(eye_ray);
	vec4 cld = render_clouds(eye_ray);
	// fragColor = vec4(cld.rgb/(0.000001+cld.a), cld.a);

	vec3 col = mix(sky, cld.rgb/(0.000001+cld.a), cld.a);
	fragColor = vec4(col, uCLOUD_OPACITY);
