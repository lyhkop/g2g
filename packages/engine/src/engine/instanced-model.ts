...
export default class InstantiateHelper {
  ...
  public instantiate() {
    ...
    console.log(
      `[Inst] instantiate() costed ${(Date.now() - startTime) / 1000}s`
    );
  }
  ...
  private instantiateInner(object: THREE.Object3D) {
    ...
    for (let i = 0; i < childCount; ++i) {
      ...
      for (let j = values.length - 1; j >= 0; --j) {
        // search from back to front for a better hit rate
        ...
        if (
          this.geometryEquals(oi.geometry, ok.geometry) &&
          MaterialUtils.materialsEquals(oi.material, ok.material)
        ) {
          if (!dict[k]) {
            dict[k] = { indexes: [k] };
      ...
      for (
        let j = nonInstIndexes.length - 1;
        !foundInstantableObject && j >= 0;
        --j
      ) {
    ...
    console.log(
      `[Inst] ${indexesToBeRemoved.length}(out of ${childCount}) objects instanced to ${instances.length} InstancedMesh`
    );
  }
  ...
  removeFromArray(arr: number[], toBeRemoved: number) {
    for (let i = arr.length - 1; i >= 0; --i) {
      // do from back to front for a better hit rate
